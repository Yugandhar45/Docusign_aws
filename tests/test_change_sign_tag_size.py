...
class Test_Change_Sign_Tag_Size:
    ...
    def test_change_sign_size(self, request):
        ...
        try:
            ...
            Util_Test.write_custom_logs("Logged in to the DocuSign Application successfully")
            utils.execute_script_with_banner("Home page is Displayed")
            home.validate_home_page()
            Util_Test.write_custom_logs("Validated the home page")
            home.click_start_button()
            Util_Test.write_custom_logs("Clicked on the start button to open the upload envelop section.")
            ...
            Util_Test.write_custom_logs("Added the recipient 2 details")
            upload.click_next_btn()
            Util_Test.write_custom_logs("Clicked on the next button")
            ...
            Util_Test.write_custom_logs("Dragged and dropped the signature tag for recipient 1")
            utils.getscreenshot('/1.Before_changing_Signature_Tag_size_100%_resolution.png')
            add_sign.validateOptionsUnderSignature()
            Util_Test.write_custom_logs("Validated the options under the signature section")
            add_sign.change_sign_tag_size()
            Util_Test.write_custom_logs("Increased the signature tag size for recipient 1")
            utils.getscreenshot('/2.After_Changing_signature_Tag_size_200%_resolution.png')
            add_sign.select_signer(constants.index_two)
            Util_Test.write_custom_logs("Selected the Recipient2 from the drop down")
            add_sign.addSignatureTag(650)
            Util_Test.write_custom_logs("Dragged and dropped the signature tag for recipient 2 ")
            add_sign.change_sign_tag_size()
            Util_Test.write_custom_logs("Increased the signature tag size for recipient 2.")
        except :
            # Log the exception and mark the test as failed
            Util_Test.write_custom_logs(f"Test  case failed")
            pytest.fail()
